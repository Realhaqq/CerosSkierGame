...
export class Rhino extends Entity {
     ...
     */
    update(gameTime: number, target: Entity): void  {
        if(this.isRunning()) {
            this.move(target);
     ...
     */
    nextAnimationFrame(gameTime: number): void  {
        if(!this.curAnimation) {
            return;
        }
     ...
     */
    checkIfCaughtTarget(target: Entity): void  {
     ...
     */
    caughtTarget(target: Entity): void  {
        target.die();
